...
Meridiano opera **dos negocios distintos, con economías distintas**. Confundirlos lleva a errores de cálculo y de propuesta. Antes de cualquier análisis o material, identificar cuál de los dos aplica.
...
Ver detalle operativo del vehículo, fees, gobernanza y red de aliados en la skill `meridiano-investor-journey` → `references/modelo-coinversion.md`.
...
- El **monto y el objetivo del inversor** definen el modelo: quien quiere ser dueño de una unidad → A; quien quiere entrar a un desarrollo con otros → B.

## Cómo se cruzan (no se excluyen)

Un mismo inversor puede empezar en el modelo A (compra una unidad) y luego entrar al B (coinvierte en un desarrollo), o viceversa. Un inversor de cartera grande suele combinar ambos. La S.A. y el Investor Pass sirven a los dos. Lo que nunca se mezcla es la **economía del cálculo**: cada operación se evalúa con el modelo que le corresponde.
...
- **Simple** (`hurdle = capital × tasa × años`): el preferente no gana interés sobre sí mismo. Favorece al estructurador (Meridiano), más fácil de explicar. La diferencia con compuesto es mínima en plazos cortos.
...
El motor elige solo según el plazo (`hurdle_tipo="auto"` en el config). Se puede forzar un tipo pasando `hurdle_tipo="simple"` o `"compuesto"` a `evaluar_coinversion`. Ejemplo del impacto: en un proyecto de 5 años, pasar de simple a compuesto transfiere ~USD 7.000 (sobre 500k de capital) del carry de Meridiano al inversor — por eso en plazos largos el compuesto es lo justo y lo esperado por inversores sofisticados.
