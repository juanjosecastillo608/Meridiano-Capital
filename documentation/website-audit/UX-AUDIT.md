...
2. **El formulario de contacto no segmenta por tipo de consulta** — el `Perfil Profesional` (sección 6.7 del plan de contenido web ya escrito) especifica un formulario segmentado ("Quiero invertir / Ya soy propietario / Soy desarrollador"), pero el formulario actual es genérico (nombre, email, país, mensaje libre). Esto ya estaba señalado como pendiente en `knowledge-base/marketing/01-aplicacion-digital.md`.
