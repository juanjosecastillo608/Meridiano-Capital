...
- Datos de contacto: los de Meridiano, desde `brand_tokens.json` o el repo. Un contacto de terceros en la fuente (otra inmobiliaria, un vendedor, un correo personal) **no se muestra**: se reemplaza por el de Meridiano, se agrega a `forbid` en `meta.json` y se registra en `text_changes`. Solo si la fuente documenta una co-comercialización (dos agencias nombradas como comercializadoras) se pregunta, porque afecta a las partes.
