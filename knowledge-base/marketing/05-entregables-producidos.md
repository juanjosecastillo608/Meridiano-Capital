...
**Automatizado desde el 2026-08-12 (D-057)**: el placeholder `[NOMBRE DEL PROPIETARIO]` de la portada ya no se reemplaza a mano — cuando un contacto real del sitio elige "Gestión de mi propiedad" **y confirma que es para renta temporal/Airbnb**, `production/app/backend/urbannit_automation.py` genera automáticamente ambas propuestas con el nombre real del contacto, listas en `.docx`/`.pdf`. El envío por email sigue pendiente (falta elegir servicio de correo, mismo patrón que el CRM D-048) — por ahora se envían a mano desde la carpeta que genera la automatización. Ver `production/app/README.md` para el detalle técnico.

**Corrección importante (D-058, 2026-08-12)**: estas dos piezas son exclusivas de renta temporal/Airbnb. "Gestión de mi propiedad" en el formulario del sitio también cubre renta tradicional (sin muebles o amoblada), que gestiona Meridiano Capital directamente, sin Urbannit — enviarlas ahí sería incorrecto. El formulario ahora distingue ambos casos con un campo adicional (`modalidad_gestion`) antes de disparar la automatización.
