Estado: CURRENT
Fuente original: meridiano-capital-identity/references/14-prompt-engine-de-marca.md
Dominio: AI

# Prompt Engine de Marca

**Nota sobre estados:** igual que la Matriz de Decision (`ai/05-matriz-de-decision.md`), este archivo es "CURRENT" en cuanto a vigencia dentro del repositorio, pero su contenido esta marcado en la fuente original como **[EXTENSION]**: sistema construido para traducir automaticamente la Direccion de Arte, la Paleta cromatica y el Sistema de Imagen de marca en prompts de generacion — no es un texto literal del Brand Guidelines oficial. Un agente que use este motor y el usuario pregunte si estas plantillas son "oficiales del manual" debe declarar que son [EXTENSION] (protocolo en `ai/02-protocolo-regla-no-definida.md`).

Usar este motor cada vez que se necesite un prompt para imagen, edicion fotografica, render, arquitectura, interiores, lifestyle, campaña publicitaria, video o motion graphics.
...
2. **Direccion de arte** (`brand/07-direccion-de-arte.md`) — luz calida/hora dorada, composicion editorial, contexto de Asuncion real.
3. **Paleta** (`brand/05-sistema-cromatico.md`) — los HEX exactos traducidos a descripcion de color-grading.
4. **Nivel de lujo / tono editorial** — precision silenciosa, nunca ostentacion obvia (`brand/07-direccion-de-arte.md`).
5. **Negative / avoid list** — lo que nunca debe aparecer (`brand/07-direccion-de-arte.md`, lista negativa).
...
```
Repetir con "atardecer / golden hour version" para el segundo momento del dia (`brand/08-sistema-de-imagen.md`).
...
Si el pedido no encaja exactamente en ninguno de estos templates, construir el prompt combinando manualmente Bloque 1 (especifico del pedido) con los Bloques 2-5 correspondientes a la marca — **nunca omitir el negative list ni improvisar una paleta de color distinta** a la definida en `brand/05-sistema-cromatico.md`.

> UNRESOLVED: `brand/05-sistema-cromatico.md`, `brand/07-direccion-de-arte.md` y `brand/08-sistema-de-imagen.md` aun no estan migrados a `knowledge-base/` (ver nota en `ai/03-sistema-de-consulta.md`). Hasta esa migracion, consultar `inventory/_raw-copies/meridiano-capital-identity/references/05-sistema-cromatico.md`, `references/07-direccion-de-arte.md` y `references/08-sistema-de-imagen.md` directamente antes de usar este motor de prompts, para verificar que los HEX y la direccion de arte citados aqui siguen vigentes.
